...
const Login = () => {
    ...
    const handleLogin = () => {
        if (LogininputValues.fullUserName.length > 2 && LogininputValues.email.endsWith('@gmail.com'))  {
            if (LogininputValues.email != "dwivedisatish06@gmail.com"){
                let users = JSON.parse(localStorage.getItem('AllUsers'))
                let userName=''
                let usermail=''
                if (users){
                    users.forEach(element => {
                        if (element.email === LogininputValues.email && element.fullUserName === LogininputValues.fullUserName){
                            userName = element.fullUserName
                            usermail = element.email
                        }
                    });
                    if (usermail === LogininputValues.email && userName === LogininputValues.fullUserName ){
                        console.log('Welcome Back')
                        navigate(`/${usermail}`)
                    }
                    else{
                        alert('wrong detail provided')
                    }
                }
                else{
                    alert('Create an account first')
                }
                setLogininputValues({ "fullUserName": '', "email": '' })
            }
            else{
    ...
    return (
        <div className="mt-[15vh]  flex  justify-center">
            <div className="container w-[60vw] bg-gray-50 rounded-lg p-8 flex flex-col border-2 border-r-1 border-blue-100">
                ...
                <h2 className="text-gray-600 text-lg font-medium title-font mb-5">Login</h2>
                ...
                <p  className="text-blue-600 text-sm mt-5">Don't have an account? 
                <NavLink to="/SignUp" >
